...
keep_headers <- c("SUBJID", 
                  ...
                  "DEP001", "DEP002", "DEP004", "DEP006", "DEP008", "DEP012", "DEP013", "DEP014", "DEP015", "DEP016", "DEP017", "DEP020", # depression
...
pheno_clean <- pheno_clean %>% 
  mutate(ADD_sum = pheno_clean %>% select(starts_with("ADD")) %>% rowSums(),
         DEP_sum = pheno_clean %>% select(starts_with("DEP")) %>% rowSums(),
         PSY_sum = pheno_clean %>% select(starts_with("PSY")) %>% rowSums())
